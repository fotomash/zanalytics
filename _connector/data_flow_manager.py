...
# data_flow_manager.py - Real-time Data Flow Awareness for ZANALYTICS
import asyncio
import json
import logging
...
class DataFlowHandler(FileSystemEventHandler):
    ...
    def _extract_symbol(self, filename: str) -> str:
        ...
        # Common patterns: EURUSD_M1, XAUUSD_TICK, etc.
    ...
    def _extract_timeframe(self, filename: str) -> str:
        ...
        if 'TICK' in filename_upper:
            return 'TICK'
        elif 'M1' in filename_upper:
            return 'M1'
        elif 'M5' in filename_upper:
            return 'M5'
        elif 'M15' in filename_upper:
            return 'M15'
        elif 'H1' in filename_upper:
            return 'H1'
        elif 'H4' in filename_upper:
            return 'H4'
        elif 'D1' in filename_upper:
            return 'D1'
        return 'UNKNOWN'
...
class DataFlowManager:
    ...
    def start_monitoring(self):
        ...
        for directory in self.watch_directories:
            if Path(directory).exists():
                handler = DataFlowHandler(self._on_data_event)
                observer = Observer()
    ...
    def _on_data_event(self, event: DataFlowEvent):
        ...
        self.logger.info(f"New data event: {event.symbol} {event.timeframe} - {event.event_type}")
