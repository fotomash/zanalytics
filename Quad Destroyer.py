            ...
            for idx in midas_data.index:
                ...
                fig.add_trace(
                    go.Scatter(
                        ...
                        hovertemplate=f'<b>MIDAS SIGNAL</b><br>' +
                                    f'Type: {setup_type}<br>' +
                                    f'Entry: ${df.loc[idx, "midas_entry"]:.2f}<br>' +
                                    f'Stop: ${df.loc[idx, "midas_stop"]:.2f}<br>' +
                                    f'Target: ${df.loc[idx, "midas_target"]:.2f}<br>' +
        ...
        if not fvg_bull.empty:
            fig.add_trace(
                go.Scatter(
                    ...
                    name='Bullish FVG',
                    hovertemplate='<b>Bullish Fair Value Gap</b><br>Price: %{y}<extra></extra>'
                ),
                row=1, col=1
            )
        ...
        if not fvg_bear.empty:
            fig.add_trace(
                go.Scatter(
                    ...
                    name='Bearish FVG',
                    hovertemplate='<b>Bearish Fair Value Gap</b><br>Price: %{y}<extra></extra>'
                ),
                row=1, col=1
            )
...
def load_enriched_timeframes():
    """Load all available timeframe data with advanced caching"""
    timeframes = {
        '1min': 'XAUUSD_1min.parquet',
        '5min': 'XAUUSD_5min.parquet', 
        '15min': 'XAUUSD_15min.parquet',
        '30min': 'XAUUSD_30min.parquet',
        '1h': 'XAUUSD_1h.parquet',
        '4h': 'XAUUSD_4h.parquet',
        '1d': 'XAUUSD_1d.parquet'
    }
    ...
    return enriched_data, data_stats
...
def main():
    ...
    with st.spinner("🔄 Loading ZANFLOW v12 enriched datasets..."):
        data, stats = load_enriched_timeframes()
    
    if not data:
        st.error("❌ No enriched data found. Please ensure XAUUSD parquet files are available.")
        return
    ...
    with st.sidebar:
        ...
        selected_timeframe = st.selectbox(
            ...
            index=len(data)-1,
            help="Select primary timeframe for analysis"
        )
    ...
    with col1:
        st.markdown(f"""
            <div class="metric-institutional">
                <div class="price-display-institutional">${current_price:.2f}</div>
                <div>XAUUSD Current Price</div>
            </div>
        """, unsafe_allow_html=True)
    ...
    if st.button("🚀 Execute ZANFLOW v12 Analysis", type="primary"):
            ...
            for signal in analysis_results['midas_signals'][-3:]:  # Show last 3
                st.markdown(f"""
                    <div class="midas-signal">
                        ...
                        <p><strong>Entry:</strong> ${signal['entry']:.2f} | <strong>Stop:</strong> ${signal['stop']:.2f} | <strong>Target:</strong> ${signal['target']:.2f}</p>
        ...
        master_chart = create_institutional_master_chart(
            analyzed_df, "XAUUSD", selected_timeframe,
        ...
        with col1:
            if st.button("📄 Export ZBAR Logs"):
                logs_json = json.dumps(zbar_logs, indent=2, default=str)
                st.download_button(
