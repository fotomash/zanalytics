# Create the fixed dashboard file
#!/usr/bin/env python3
"""
ZanFlow Ultimate Trading Dashboard
...
def load_data(data_folder="./data"):
    ...
    if os.path.exists(data_folder):
        # Get all subdirectories (trading pairs)
        for pair_dir in os.listdir(data_folder):
            pair_path = os.path.join(data_folder, pair_dir)
            if os.path.isdir(pair_path):
                # Load CSV files from each pair directory
                for file in os.listdir(pair_path):
                    if file.endswith('.csv'):
                        try:
                            file_path = os.path.join(pair_path, file)
                            df = pd.read_csv(file_path)
                            
                            # Create unique identifier
                            data_key = f"{pair_dir}/{file.replace('.csv', '')}"
                            ...
                            if 'timestamp' in df.columns:
                                ...
                                if 'open' not in df.columns:
                                    df['open'] = df.get('open', df.get('mid', df.get('close', 0)))
                                if 'high' not in df.columns:
                                    df['high'] = df.get('high', df.get('mid', df.get('close', 0)))
                                if 'low' not in df.columns:
                                    df['low'] = df.get('low', df.get('mid', df.get('close', 0)))
                                if 'close' not in df.columns:
                                    df['close'] = df.get('close', df.get('mid', 0))
                                ...
                                print(f"✓ Loaded {data_key}: {len(df)} rows")
                        except Exception as e:
                            print(f"Error loading {pair_dir}/{file}: {e}")
...
def generate_microstructure_commentary(df: pd.DataFrame, pair_name: str) -> str:
    ...
    if 'micro_order_flow_imbalance' in df.columns:
        ...
        if avg_imbalance > 0.1:
            commentary.append(f"- **Dominant Flow