...
# ─── Parquet base directory from secrets ────────────────────────────────
PARQUET_DATA_DIR = Path(st.secrets.get("PARQUET_DATA_DIR", "./parquet"))
...
class EnhancedSMCDashboard:
    ...
    def scan_available_symbols(self):
        """Scan PARQUET_DATA_DIR for symbol subfolders."""
        symbols = [f.name for f in PARQUET_DATA_DIR.iterdir() if f.is_dir()]
        self.available_symbols = sorted(symbols)

    def list_timeframes(self, symbol: str):
        """Return list of timeframes (e.g. '1d', '1h', '1min') for a symbol."""
        folder = PARQUET_DATA_DIR / symbol
        tfs = []
        if folder.exists():
            for f in folder.glob(f"{symbol}_*.parquet"):
                tf = f.stem.replace(f"{symbol}_", "")
                tfs.append(tf)
        return sorted(tfs)
    ...
    def run(self):
        ...
        # Symbol selector and timeframe selector
        selected_symbol = st.selectbox("📈 Select Trading Symbol", self.available_symbols)
        st.session_state.selected_symbol = selected_symbol

        # Timeframe selector (populate after symbol choice)
        tfs = self.list_timeframes(selected_symbol)
        if not tfs:
            st.error("No Parquet files found for this symbol.")
            return
        selected_tf = st.selectbox("⏳ Timeframe", tfs)
    ...
    def create_advanced_chart(self) -> go.Figure:
        ...
        parquet_path = PARQUET_DATA_DIR / symbol / f"{symbol}_{tf}.parquet"
        ...
        # ── Normalize columns & guarantee timestamp ────────────────────────────
        df.columns = [c.strip().lower() for c in df.columns]

        # Bring index to column if needed
        ...
        if "timestamp" not in df.columns:
            if "index" in df.columns:
                ...
                for c in df.columns:
                    if c.startswith(("time", "date")):
                        df.rename(columns={c: "timestamp"}, inplace=True)
                        break

        if "timestamp" not in df.columns:
            st.error(f"'timestamp' column missing in {parquet_path.name}")
            return go.Figure()

        # Ensure tick volume exported as volume
        ...
        if missing:
            st.error(f"Missing columns {missing} in {parquet_path.name}")
            return go.Figure()
    ...
    def add_order_blocks(self, fig: go.Figure, dates: pd.DatetimeIndex, prices: np.ndarray):
        ...
        ob_start = 50
        ob_end = 70
        if len(prices) < 71:  # Need at least up to index 70
            return
        if max(ob_start, ob_end) >= len(prices):
            return
        ob_high = max(prices[ob_start:ob_end]) + 1
        ob_low = min(prices[ob_start:ob_end]) - 0.5

        fig.add_shape(
            type="rect",
            x0=dates.iloc[ob_start], x1=dates.iloc[ob_end],
        ...
        # Add annotation
        fig.add_annotation(
            x=dates.iloc[ob_start + 10] if (ob_start + 10) < len(dates) else dates.iloc[-1],
            y=ob_high + 0.5,
            text="Bullish OB",
            showarrow=True,
            arrowhead=2,
    ...
    def add_fair_value_gaps(self, fig: go.Figure, dates: pd.DatetimeIndex, prices: np.ndarray):
        ...
        fvg_index = 100
        if len(prices) < 106:  # Need at least fvg_index + 5
            return
        if fvg_index >= len(prices):
            return

        left = max(fvg_index - 5, 0)
        right = min(fvg_index + 5, len(prices) - 1)
        ...
        fig.add_shape(
            type="rect",
            x0=dates.iloc[left], x1=dates.iloc[right],
    ...
    def add_liquidity_zones(self, fig: go.Figure, dates: pd.DatetimeIndex, prices: np.ndarray):
        ...
        fig.add_trace(
            go.Scatter(
                x=[dates[grab_index]] if grab_index < len(dates) else [dates.iloc[-1]],
                y=[liq_level + 0.5],
                mode='markers',
                marker=dict(
    ...
    def add_points_of_interest(self, fig: go.Figure, dates: pd.DatetimeIndex, prices: np.ndarray):
        ...
        # Ensure enough data for hardcoded indexes
        if len(prices) < 130:
            return
        # Define POIs
        pois = [
    ...
    def add_market_structure(self, fig: go.Figure, dates: pd.DatetimeIndex, prices: np.ndarray):
        ...
        # Check if enough data for the largest index used
        if len(prices) < 181:
            return
        # Plot swing highs
        ...
        # Add trend lines
        if len(swing_lows) > 1 and max(swing_lows[0], swing_lows[-1]) < len(dates):
            fig.add_trace(
                go.Scatter(
