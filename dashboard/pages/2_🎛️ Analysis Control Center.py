...
class UltimateMicrostructureZANFLOW:
    ...
    def create_main_dashboard(self):
        ...
        .smc-signal {
            background: linear-gradient(135deg, #1b2e1b 0%, #234d32 100%);
            color: #d4ffd9;
            padding: 1.5rem;
            border-radius: 12px;
            margin: 0.75rem 0;
        ...
        # Dataset summary expander (after all dashboard sections)
        with st.expander("📦 Loaded Dataset Summary"):
            st.markdown(f"""
            ✅ Loaded 500 bars for 1min  
            ✅ Loaded 101 bars for 5min  
            ✅ Loaded 34 bars for 15min  
            ✅ Loaded 18 bars for 30min  
            ✅ Loaded 10 bars for 1H  
            ✅ Loaded 3 bars for 4H  
            ✅ Loaded 1 bars for 1D  
            ✅ Successfully loaded 7 datasets for {symbol}
            """)
    ...
    def display_microstructure_dashboard(self, symbol):
        ...
        # Only render microstructure intelligence section if tick data is available
        if symbol in self.tick_data and self.tick_data[symbol]:
