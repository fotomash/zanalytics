...
class QuantumMicrostructureAnalyzer:
    ...
    def infer_volume_from_ticks(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        df['tick_density'] = 1000 / (df['tick_interval_ms'] + 1)
        ...
        df['volume_score'] = (
            params['tick_density_weight'] * StandardScaler().fit_transform(df[['tick_density']]).flatten() +
            params['spread_change_weight'] * StandardScaler().fit_transform(df[['spread_change']]).flatten() +
            params['price_movement_weight'] * StandardScaler().fit_transform(df[['price_velocity']]).flatten()
        )
        
        # Convert to actual volume estimate (normalized to reasonable range)
        df['inferred_volume'] = np.exp(df['volume_score']) * 100
        df['inferred_volume'] = df['inferred_volume'].clip(lower=0)
        ...
        for idx in spike_indices:
            if idx + 10 < len(df):  # Need future data to confirm reversal
                ...
                if reversal_ratio < params['price_recovery_threshold']:
                    reversal_time = (future_window[future_window['spread'] == min_future_spread]['timestamp'].iloc[0] - 
    ...
    def detect_quote_stuffing(self, df: pd.DataFrame) -> List[Dict]:
        ...
        window_size = 50
        df['update_rate'] = 1000 / (df['tick_interval_ms'] + 1)
    ...
    def detect_layering(self, df: pd.DataFrame) -> List[Dict]:
        ...
        for i in range(window, len(df) - window):
            ...
            if abs(segment['pressure_imbalance'].mean()) > segment['pressure_imbalance'].std() * 2:
    ...
    def calculate_vpin(self, df: pd.DataFrame, bucket_size: int = 50) -> pd.DataFrame:
        ...
        # Pad the end
        vpin_values.extend([vpin_values[-1]] * (len(df) - len(vpin_values)))
    ...
    def detect_market_regime(self, df: pd.DataFrame) -> str:
        ...
        spread_volatility = df['spread'].tail(100).std() / df['spread'].mean()
    ...
    def create_advanced_dashboard(self, df: pd.DataFrame, selected_file: str):
        ...
        with st.expander("📊 Market Quality Metrics"):
            ...
            with col1:
                st.metric("Avg Spread", f"{df['spread'].mean():.2f}")
                st.metric("Spread Volatility", f"{df['spread'].std():.2f}")
                st.metric("Price Efficiency", f"{1 - df['spread'].mean()/df['price_mid'].mean():.4f}")
            ...
            with col3:
                ...
                st.metric("Price Impact", f"{df['price_velocity'].mean():.6f}")
...
if __name__ == "__main__":
    ...
    valid_files = [f for f in os.listdir(tick_files_directory) 
                   if f.endswith(('.csv', '.txt')) and not f.startswith('._')]
    
    if not valid_files:
        st.sidebar.error("No valid tick files found")
        st.stop()
