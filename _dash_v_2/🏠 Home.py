...
class ZanalyticsDashboard:
    ...
    def display_home_page(self, data_sources):
        ...
        connector = ZanflowDataConnector(host="mm20.local", port=8080)
        ...
        if data:
            st.markdown(f"### 📊 Real-Time Market Data for {selected_symbol}")
            cols = st.columns(4)
            cols[0].metric("Bid", f"{data['bid']:.5f}")
            cols[1].metric("Ask", f"{data['ask']:.5f}")
            cols[2].metric("Spread", f"{data['spread']:.5f}")

            enriched = data.get('enriched', {})
            if enriched:
                cols[3].metric("RSI", f"{enriched.get('rsi', 'N/A'):.2f}")
                st.markdown(f"**Trend:** {enriched.get('trend', 'N/A')}")
