...
class EconomicDataManager:
    ...
    def get_dxy_data(self) -> Optional[pd.DataFrame]:
        ...
        try:
            ...
            if not hist.empty:
                return hist.tail(200)  # Last 200 M15 bars (limit bars to 200)
            return None
        except Exception:
            return None
...
class ZanalyticsDashboard:
    ...
    def display_home_page(self, data_sources):
        ...
        if xau_ticks_path.exists():
            try:
                ...
                # Limit to last 200 bars for charting if more than 200
                if len(df_ticks) > 200:
                    df_ticks = df_ticks.tail(200)
        ...
        try:
            ...
            if parquet_file:
                ...
                if "timestamp" in df_3d.columns and "close" in df_3d.columns:
                    ...
                    # Limit to last 200 bars for 3D plot
                    if len(df_3d) > 200:
    ...
    def create_multi_asset_3d_chart(self, data_sources):
        ...
        for idx, (asset, colorscale) in enumerate(assets.items()):
            ...
            # Limit to last 200 bars for 3D chart
            if len(df) > 200:
                df = df.sort_values("timestamp").tail(200).copy()
            else:
